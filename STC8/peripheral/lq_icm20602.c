...
uint8_t ICM_Set_LPF(uint16_t lpf)
{
	...
	return ICM_Write_Byte(ICM_CFG_REG,dat);//设置数字低通滤波器  
}
...
uint8_t ICM_Set_Rate(uint16_t rate)
{
	...
	if(rate<4)rate=4;
	dat=1000/rate-1;
	dat=ICM_Write_Byte(ICM_SAMPLE_RATE_REG,dat);	    //设置数字低通滤波器
...
uint8_t ICM_Write_Byte(uint8_t reg,uint8_t value)
{
    ...
    return SPI_ReadWriteNByte(buff, buff, 2); //发送buff里数据，并采集到 buff里  
}
